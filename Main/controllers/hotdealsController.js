app.controller("HotDealsController", function ($scope, $http, $routeParams) {
    ...
    function loadProducts(categoryIds, brandIds, page = 1) {
        let url = "../Core/Controller/ProductController.php?action=PromotedItemsReadByFilter"
        ...
        $http.get(url).then(function (response) {
            ...
            $scope.PromotedProducts = res.data || [];
            $scope.PromotedProducts.forEach(function (product) {
                product.Qty = 1;
            });
    ...
    $scope.addToCart = function (currentProductId, newQty) {
        const currentProduct = $scope.PromotedProducts.find(p => p.product_id === currentProductId);
